<script lang="ts">
  ...
  onMount(() => {
    if (browser) {
      ...
      // optional: if already signed in, redirect
      if (session?.user) {
...
{#if session?.user}
  <div class="p-8">
    ...
    <button class="mt-4 px-4 py-2 bg-gray-800 text-white rounded" on:click={signOut}>Sign out</button>
  </div>
{:else}
    ...
    <div class="relative w-full h-screen">
      <div
        class="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2
               w-[426px] h-[190px] flex flex-col items-center justify-center bg-opacity-75"
      >
        ...
        >
          <span class="absolute inset-0 -translate-y-[2px] -translate-x-[0px] rounded-md bg-[#303784]"></span>
          <span class="absolute inset-0 translate-y-[0px] rounded-md bg-[#5865f2]"></span>
